...
// NAMESTR is a dynamic string to be used as name, ID must be a static name (not string, like in D3_PROF_BeginCPUSample())
...
// ID_NAME is the ID from D3_PROF_BeginCPUSampleDynamic or the NAME from D3_PROF_BeginCPUSample
#define D3P_EndCPUSample(ID_NAME) \
	TracyCZoneEnd(_D3_TRACY_ ## ID_NAME)

// scoped sample for whole function with its backtrce, using the function name as name
